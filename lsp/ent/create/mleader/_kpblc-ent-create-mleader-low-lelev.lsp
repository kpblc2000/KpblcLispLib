  ...
  (if (cdr (assoc "base" lst))
    (progn (if (or (not (cdr (assoc "where" lst)))
           ...
           (if (not (cdr (assoc "pt" lst)))
             (setq lst (_kpblc-list-add-or-subst lst
                                                 "pt"
           ...
           (setq lst (_kpblc-list-add-or-subst lst "base" (_kpblc-conv-2d-to-3d (cdr (assoc "base" lst))))
                 ...
                 res (vla-addmleader (cdr (assoc "where" lst))
                                     (vlax-make-variant
           ...
           (vla-put-textstring res
                               (_kpblc-conv-list-to-string (vl-remove nil (mapcar (function (lambda (x) (cdr (assoc x lst)))) '("upstring" "lowstring")))
                                                           "\\P"
           ...
           (vla-setleaderlinevertices res
                                      (vla-getleaderindex res (1- (vla-get-leadercount res)))
                                      (vlax-make-variant
                                        (vlax-safearray-fill (vlax-make-safearray vlax-vbdouble (cons 0 5))
